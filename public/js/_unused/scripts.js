(function() {
    ...
    /**
     * Add selected text to the title
     */
    $(document).on('change', 'select.add-value-to-title', function() {
        var scope = $(this),
            id = scope.data('id');

        scope.find(":selected").each(function () {
            $('.get-text-from-select' + id).val($(this).text());
        });
     });
